...
// "use client" + Suspense: situs ini output:'export' (statis penuh), jadi
// pesan error/notice harus dibaca dari URL di BROWSER setelah redirect dari
// Pages Function /aman-engine/login, bukan lewat searchParams di server.

const CHECKOUT = "https://lynk.id/adya.malik/6y6m9djlkywg/checkout";
const WA = `https://wa.me/6282210768038?text=${encodeURIComponent("Halo AMAN Digital, saya mau tanya tentang akses AMAN Engine.")}`;
...
function FormLogin() {
  ...
  return (
    <div className="w-full max-w-sm text-center">
      ...
      <h1 className="mt-4 text-2xl font-black">AMAN Engine</h1>
      ...
      <form method="POST" action="/aman-engine/login" className="mt-6 flex flex-col gap-3">
        <label htmlFor="code" className="sr-only">Kode akses</label>
        <input
          type="text"
          id="code"
          name="code"
          placeholder="AMAN-XXXXXXXX"
          required
          autoFocus
          maxLength={40}
          autoComplete="off"
...
export default function AmanEngineLoginPage() {
  return (
