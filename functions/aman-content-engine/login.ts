...
const RATE_SCOPE = "content";

const redirect = (reason: string) =>
  new Response(null, { status: 303, headers: { Location: `/aman-content-engine/masuk?e=${reason}` } });
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const result = await checkDevice(context.env.AMAN_LEDGER, code, cookieToken, "aman-content-engine");
  ...
  const headers = new Headers({ Location: "/aman-content-engine/app/" });
