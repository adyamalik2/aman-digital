...
const RATE_SCOPE = "poster";

const redirect = (reason: string) =>
  new Response(null, { status: 303, headers: { Location: `/aman-poster/masuk?e=${reason}` } });
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const result = await checkDevice(context.env.AMAN_LEDGER, code, cookieToken, "aman-poster");
  ...
  const headers = new Headers({ Location: "/aman-poster/app/" });
